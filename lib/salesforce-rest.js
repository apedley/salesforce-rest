...
module.exports = {
  ...
  post: function(path, data, callback) {
    if (!this._options.instanceURL || !this._options.accessToken || !path) {
      throw new Error("Options not set");
    }
    ...
    var req = https.request(options, function(res) {
      ...
      res.on('end', function() {
        ...
        data = {status: res.statusCode, data: JSON.parse(data)};
        // run callback
        callback(null, data);
      });
  ...
  put: function(objectName, id, data, callback) {
    if (!this._options.instanceURL || !objectName || !id || !data || !this._options.accessToken) {
      throw new Error('Options not set');
    }
    var path = '/services/data/v20.0/sobjects/' + objectName + '/' + id;
    data = JSON.stringify(data);
  ...
  delete: function(objectName, id, callback) {
    if (!objectName || !id || !this._options.accessToken) {
      throw new Error('Options not set');
    }
    var path = '/services/data/v20.0/sobjects/' + objectName + '/' + id.toString();
